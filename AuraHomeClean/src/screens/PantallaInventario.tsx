...
import { BebidasIcon, CongeladosIcon, FrutasIcon, LacteosIcon, ProteinasIcon, VegetalesIcon } from "../assets/icons/categories";
...
export default function PantallaInventario({ profile }: PantallaInventarioProps) {
...
function HomeScreenContainer({ homeId }: { homeId?: string | number }) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      {homeId ? (
        <View style={styles.homeBadge}>
          <Text style={styles.homeBadgeText}>Hogar: {homeId}</Text>
        </View>
      ) : null}
    </SafeAreaView>
  );
      ...
      <View style={styles.categoryGrid}>
        {categoryCards.map(({ id, title, items, storage, color /*, Icon */ }) => (
          <TouchableOpacity
            ...
            onPress={() => (navigation as any).navigate('CategoryProducts', { categoryId: id, categoryLabel: title })}
          > 
...
const styles = StyleSheet.create({
  ...
  loadingText: { fontSize: 16, color: palette.muted, fontWeight: "600" },
  homeBadge: {
    position: "absolute",
    right: 20,
    bottom: 110,
  ...
  homeBadgeText: { color: palette.text, fontSize: 12, fontWeight: "600" },
    bannerCard: {
